...
/**
 * Webhook do Whapi.Cloud (WhatsApp).
 *
 * Migramos do Evolution para o Whapi por segurança e simplicidade: o Whapi
 * entrega a mídia JÁ DESCRIPTOGRAFADA em um link HTTPS direto (`...link`),
 * então não precisamos mais do passo de descriptografia da mídia `.enc`.
 *
 ...
 *   Events: marque "messages" (mensagens recebidas).
 *   Mode:   "body" (corpo completo).
 *
 * Segurança: a chamada DEVE trazer um token secreto que casa com o secret
 * `WHAPI_WEBHOOK_TOKEN`. Ele pode vir por:
 *   - querystring:  ...?token=<token>
 *   - header:       Authorization: Bearer <token>
 *   - header:       X-Webhook-Token: <token>
 * Se `WHAPI_WEBHOOK_TOKEN` estiver definido e o token não casar, respondemos
 * 401 e NADA é processado. (Se o secret não existir, o webhook aceita — útil
 * só para um primeiro teste; defina o secret em produção.)
 *
 * A empresa é resolvida pelo TELEFONE DO REMETENTE (colaborador cadastrado em
 * `profiles.whatsapp`). Mensagens de números não cadastrados são ignoradas.
 *
 * Formato típico do payload do Whapi:
 *   {
 *     "messages": [
 *       {
 *         "id": "ABGG...",
 ...
 * Retorna os bytes + mimetype, ou null se falhar.
 */
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        // O Whapi envia `messages: [...]`. Aceita também 1 mensagem solta.
        ...
        for (const msg of messages) {
          ...
          if (!companyId) {
            console.warn(
              "[whapi webhook] empresa não resolvida. sender=",
              sender,
            );
            ...
            results.push({
              status: "ignorado",
              reason:
                "colaborador (whatsapp) não cadastrado em nenhuma empresa",
            });
            continue;
          }
          ...
          if (media) {
            ...
            attachmentUrl = await uploadComprovante(
              companyId,
              media.bytes,
              media.mime,
            );
